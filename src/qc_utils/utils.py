...
def merge_quality_control_files(
    ...
    """Find all *quality_control*.json files under each of search_paths, merge
    them, and write the result to output_path.
    ...
    ----------
    search_paths : list[Path]
        Directories to search recursively for ``*quality_control*.json`` files.
    output_path : Path
    ...
    ------
    ValueError
        If no valid ``*quality_control*.json`` files are found under any of
        ``search_paths``.
    """
    qc_objects = []
    ...
    if not qc_objects:
        raise ValueError(
            f"No valid *quality_control*.json files found under {search_paths}"
        )
